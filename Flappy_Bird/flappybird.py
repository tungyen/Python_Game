...
game_images = {}
game_sounds = {}
player = 'gallery/images/yellowbird-upflap.png'
...
def getRandomPipe(score):
    ...
    if score < 10:
        y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 1.4 * offset))
    elif score >= 10 & score < 30:
        y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 1.7 * offset))
    elif score >= 30 & score < 50:
        y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 2 * offset))
    elif score >= 50 & score < 100:
        y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 2.2 * offset))
    else:
        y2 = offset + random.randrange(0, int(screen_height - game_images['base'].get_height() - 2.5 * offset))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                screen.blit(game_images['base'], (base_x, ground_y + 35))
...
def mainGame():
    ...
    newPipe3 = getRandomPipe(score)

    upperPipes = [
        {'x': screen_width + 200, 'y': newPipe1[0]['y']},
        {'x': screen_width + 200 + (screen_width / 3), 'y': newPipe2[0]['y']},
        {'x': screen_width + 200 + (2 * screen_width / 3), 'y': newPipe2[0]['y']}
    ]

    lowerPipes = [
        {'x': screen_width + 200, 'y': newPipe1[1]['y']},
        {'x': screen_width + 200 + (screen_width / 3), 'y': newPipe2[1]['y']},
        {'x': screen_width + 200 + (2 * screen_width / 3), 'y': newPipe2[1]['y']}
    ]
